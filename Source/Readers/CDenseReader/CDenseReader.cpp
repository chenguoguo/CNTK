...
namespace Microsoft {
	namespace MSR {
		namespace CNTK {
			...
			void DenseBinaryInput<ElemType>::StartDistributedMinibatchLoop(size_t mbSize, size_t subsetNum, size_t numSubsets) {
				...
				if (!m_bQueueBufferAllocated){
					...
					for (size_t c = 0; c < unzipQueueLen; c++) {
						void* unzipDataBuffer = malloc((sizeof(int32_t) * m_totalDim * m_blockSampleCnt * 2)); //sample cnt
			...
			template<class ElemType>
			void DenseBinaryInput<ElemType>::Unzip(void * input, void * output, int inputSize, int outputSize)
			{
				z_stream infstream;
			...
			template<class ElemType>
			bool CDensereader<ElemType>::GetMinibatch(StreamMinibatchInputs& matrices) {
				//timer = clock();
#if DEBUG
