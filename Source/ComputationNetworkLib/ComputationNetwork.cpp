...
void ComputationNetwork::PerformSVDecomposition(const map<wstring, float>& SVDConfig, size_t alignedSize)
{
    ...
    for (auto& group : nodeGroups)
    {
        ...
        for (const auto& name : group.first)
        {
            ...
            ComputationNodeBasePtr outputNode = GetOutputNode(name);
            auto pOutputNode = dynamic_pointer_cast<TimesNode<ElemType>>(outputNode);
            if (pOutputNode != nullptr)
            {
                // Change the hierarchy of nodes to ensure multiplication order
                // U*(V*X)
                shared_ptr<ComputationNode<ElemType>> pTimes = New<TimesNode<ElemType>>(m_deviceId, name + L".SVD");
                pTimes->AttachInputs({ pLeft, pOutputNode });

                InsertNode(pOutputNode->GetName(), pTimes);
                ReplaceLeafNode(name, pRight);
            }
            else
            {
                // Defaut multiplication order
