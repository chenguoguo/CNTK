...
CPUMatrix<ElemType>::CPUMatrix(const CPUMatrix<ElemType>& deepCopyFrom)
{
    ...
    */
    ...
    m_base = make_shared<BaseMatrixStorage<ElemType>>(GetFormat(), GetComputeDeviceId());
...
CPUMatrix<ElemType>& CPUMatrix<ElemType>::operator=(CPUMatrix<ElemType>&& moveFrom)
{
    if (this != &moveFrom)
    {
        ...
        if (OwnBuffer() && m_base != nullptr)
            m_base = nullptr; // always delete the data pointer since we will use the pointer from moveFrom
...
#pragma omp parallel for
    for (long j = 0; j < n; j++)
    {
        ...
        memcpy(BufferPointer() + j * numRows, a.BufferPointer() + j * k + startIndex, sizeof(ElemType) * numRows);
...
void CPUMatrix<ElemType>::SetValue(const ElemType v)
{
    ...
    if (isFinite && v == 0)
    {
        memset(BufferPointer(), 0, sizeof(ElemType) * GetNumElements());
    }
    else
    {
...
void CPUMatrix<ElemType>::SetValue(const size_t numRows, const size_t numCols, ElemType* pArray, const size_t matrixFlags)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                memcpy(BufferPointer(), pArray, GetNumElements() * sizeof(ElemType));
            }
...
#pragma omp parallel for
                    ...
                    {
#ifdef USE_ACML
                        dcopy((int) numRows, reinterpret_cast<double*>(pArray + j), (int) numCols, reinterpret_cast<double*>(BufferPointer() + LocateColumn(j)), 1);
#else
                        cblas_dcopy((int) numRows, reinterpret_cast<double*>(pArray + j), (int) numCols, reinterpret_cast<double*>(BufferPointer() + LocateColumn(j)), 1);
#endif
                    }
                }
                else
...
#ifdef USE_ACML
                            scopy((int) numRows, reinterpret_cast<float*>(pArray + j), (int) numCols, reinterpret_cast<float*>(BufferPointer() + LocateColumn(j)), 1);
#else
                            cblas_scopy((int) numRows, reinterpret_cast<float*>(pArray + j), (int) numCols, reinterpret_cast<float*>(BufferPointer() + LocateColumn(j)), 1);
#endif
                        }
                    }
                }
...
void CPUMatrix<ElemType>::SetUniformRandomValue(const ElemType low, const ElemType high, unsigned long seed)
{
    ...
    for (long i = 0; i < (m & ~3); i += 4)
    {
        BufferPointer()[i] = r(generator);
        BufferPointer()[i + 1] = r(generator);
        BufferPointer()[i + 2] = r(generator);
        BufferPointer()[i + 3] = r(generator);
    }
    // handle remaining stuffs
    for (long i = m & ~3; i < m; i++)
    {
...
ElemType CPUMatrix<ElemType>::Adagrad(CPUMatrix<ElemType>& gradients, const bool needAveMultiplier)
{
    ...
    ElemType *a = BufferPointer(), *d_v = gradients.BufferPointer();
                                    ...
                                    ElemType adaMul)
{
    ...
    ElemType* grad = gradients.BufferPointer();
    ElemType* smoothAda = BufferPointer();
    ElemType* smoothMom = BufferPointer() + n;
    ElemType* val = functionValues.BufferPointer();
#pragma omp parallel for
                                      ...
                                      const bool needAveMultiplier)
{
    ...
    size_t n = gradients.GetNumElements();
    ElemType* curr_grad = gradients.BufferPointer();
...
ElemType* CPUMatrix<ElemType>::CopyToArray() const
{
    ...
    if (numElements != 0)
    {
        ...
        memcpy(arrayCopyTo, BufferPointer(), sizeof(ElemType) * numElements);
        return arrayCopyTo;
    }
    else
    {
...
#pragma omp parallel for
    ...
    {
        if (BufferPointer()[i] > threshold)
            BufferPointer()[i] -= threshold;
        else if (BufferPointer()[i] < -threshold)
            BufferPointer()[i] += threshold;
        else
            BufferPointer()[i] = 0;

        if (BufferPointer()[i + 1] > threshold)
            BufferPointer()[i + 1] -= threshold;
        else if (BufferPointer()[i + 1] < -threshold)
            BufferPointer()[i + 1] += threshold;
        else
            BufferPointer()[i + 1] = 0;

        if (BufferPointer()[i + 2] > threshold)
            BufferPointer()[i + 2] -= threshold;
        else if (BufferPointer()[i + 2] < -threshold)
            BufferPointer()[i + 2] += threshold;
        else
            BufferPointer()[i + 2] = 0;

        if (BufferPointer()[i + 3] > threshold)
            BufferPointer()[i + 3] -= threshold;
        else if (BufferPointer()[i + 3] < -threshold)
            BufferPointer()[i + 3] += threshold;
        else
            BufferPointer()[i + 3] = 0;
    }
...
ElemType CPUMatrix<ElemType>::SumOfAbsElements() const
{
    ...
    {
#ifdef USE_ACML
        return (ElemType) dasum((int) GetNumElements(), reinterpret_cast<double*>(BufferPointer()), 1);
#else
        return (ElemType) cblas_dasum((int) GetNumElements(), reinterpret_cast<double*>(BufferPointer()), 1);
#endif
    }
    else
    {
#pragma warning(suppress : 4244)
#ifdef USE_ACML
        return sasum((int) GetNumElements(), reinterpret_cast<float*>(BufferPointer()), 1);
#else
        return cblas_sasum((int) GetNumElements(), reinterpret_cast<float*>(BufferPointer()), 1);
#endif
    }
}
...
#pragma omp parallel for reduction(+ : sum)
    for (long i = 0; i < (m & ~3); i += 4)
    {
        sum += BufferPointer()[i] + BufferPointer()[i + 1] + BufferPointer()[i + 2] + BufferPointer()[i + 3];
    }
    // handle remaining stuffs
    for (long i = m & ~3; i < m; i++)
    {
...
#pragma omp parallel for
            ...
            {
#ifdef USE_ACML
                c(0, j) = (ElemType) dnrm2(m, reinterpret_cast<double*>(us.BufferPointer() + us.LocateColumn(j)), 1);
#else
                c(0, j) = (ElemType) cblas_dnrm2(m, reinterpret_cast<double*>(us.BufferPointer() + us.LocateColumn(j)), 1);
#endif
            }
        }
        else
...
#pragma warning(suppress : 4244)
#ifdef USE_ACML
                c(0, j) = snrm2(m, reinterpret_cast<float*>(us.BufferPointer() + us.LocateColumn(j)), 1);
#else
                c(0, j) = cblas_snrm2(m, reinterpret_cast<float*>(us.BufferPointer() + us.LocateColumn(j)), 1);
#endif
            }
        }
    }
...
#pragma omp parallel for
            ...
            {
#ifdef USE_ACML
                c(i, 0) = dnrm2(n, reinterpret_cast<double*>(us.BufferPointer() + i), m);
#else
                c(i, 0) = cblas_dnrm2(n, reinterpret_cast<double*>(us.BufferPointer() + i), m);
#endif
            }
        }
        else
...
#pragma warning(suppress : 4244)
#ifdef USE_ACML
                c(i, 0) = snrm2(n, reinterpret_cast<float*>(us.BufferPointer() + i), m);
#else
                c(i, 0) = cblas_snrm2(n, reinterpret_cast<float*>(us.BufferPointer() + i), m);
#endif
            }
        }
    }
...
#pragma omp parallel for reduction(+ : v)
    for (long i = 0; i < (m & ~3); i += 4)
    {
        v += BufferPointer()[i] * BufferPointer()[i] + BufferPointer()[i + 1] * BufferPointer()[i + 1] + BufferPointer()[i + 2] * BufferPointer()[i + 2] + BufferPointer()[i + 3] * BufferPointer()[i + 3];
    }
    // handle remaining stuffs
    for (long i = m & ~3; i < m; i++)
    {
        v += BufferPointer()[i] * BufferPointer()[i];
    }
...
CPUMatrix<ElemType>& CPUMatrix<ElemType>::AssignNumOfDiff(const CPUMatrix<ElemType>& a, const CPUMatrix<ElemType>& b, bool searchInCol)
{
    ...
    else
    {
        ...
        const ElemType* curCol = b.BufferPointer();
                                                 ...
                                                 ElemType beta, CPUMatrix<ElemType>& c)
{
    ...
    {
#ifdef USE_ACML
        dgemm(transA, transB, m, n, k, alpha, reinterpret_cast<double*>(a.BufferPointer()), lda, reinterpret_cast<double*>(b.BufferPointer()), ldb, beta, reinterpret_cast<double*>(c.BufferPointer()), ldc);
#else
        cblas_dgemm((CBLAS_ORDER) BLAS_COLMAJOR mklTransA, mklTransB, m, n, k, alpha, reinterpret_cast<double*>(a.BufferPointer()), lda, reinterpret_cast<double*>(b.BufferPointer()), ldb, beta, reinterpret_cast<double*>(c.BufferPointer()), ldc);
#endif
    }
    else
    {
#pragma warning(suppress : 4244)
#ifdef USE_ACML
        sgemm(BLAS_COLMAJOR transA, transB, m, n, k, alpha, reinterpret_cast<float*>(a.BufferPointer()), lda, reinterpret_cast<float*>(b.BufferPointer()), ldb, beta, reinterpret_cast<float*>(c.BufferPointer()), ldc);
#else
        cblas_sgemm((CBLAS_ORDER) BLAS_COLMAJOR mklTransA, mklTransB, m, n, k, alpha, reinterpret_cast<float*>(a.BufferPointer()), lda, reinterpret_cast<float*>(b.BufferPointer()), ldb, beta, reinterpret_cast<float*>(c.BufferPointer()), ldc);
#endif
    }
}
...
void CPUMatrix<ElemType>::SVD(const CPUMatrix<ElemType>& A, CPUMatrix<ElemType>& SIGMA, CPUMatrix<ElemType>& U, CPUMatrix<ElemType>& VT, CPUMatrix<ElemType>& W)
{
    ...
    {
#ifdef USE_ACML
        dgesvd('A', 'A', (int) m, (int) n, reinterpret_cast<double*>(A.BufferPointer()), (int) lda, reinterpret_cast<double*>(SIGMA.BufferPointer()), reinterpret_cast<double*>(U.BufferPointer()), (int) ldu, reinterpret_cast<double*>(VT.BufferPointer()), (int) ldvt, &info);
#elif defined(USE_MKL)
        double wkopt;
        int lwork = -1;
        dgesvd("All", "All", &m, &n, reinterpret_cast<double*>(A.BufferPointer()), &lda, reinterpret_cast<double*>(SIGMA.BufferPointer()), reinterpret_cast<double*>(U.BufferPointer()), &ldu, reinterpret_cast<double*>(VT.BufferPointer()), &ldvt, &wkopt, &lwork, &info);
        lwork = (int) wkopt;
        W.Resize(lwork, 1);
        dgesvd("All", "All", &m, &n, reinterpret_cast<double*>(A.BufferPointer()), &lda, reinterpret_cast<double*>(SIGMA.BufferPointer()), reinterpret_cast<double*>(U.BufferPointer()), &ldu, reinterpret_cast<double*>(VT.BufferPointer()), &ldvt, reinterpret_cast<double*>(W.BufferPointer()), &lwork, &info);
#else
        std::vector<double> superb(std::max(std::min(m, n) - 1, 1));
        info = LAPACKE_dgesvd(BLAS_COLMAJOR 'A', 'A', (int) m, (int) n, reinterpret_cast<double*>(A.BufferPointer()), (int) lda, reinterpret_cast<double*>(SIGMA.BufferPointer()),
            reinterpret_cast<double*>(U.BufferPointer()), (int) ldu, reinterpret_cast<double*>(VT.BufferPointer()), (int) ldvt, &superb[0]);
#endif
    }
    else
    {
#ifdef USE_ACML
#pragma warning(suppress : 4244)
        sgesvd('A', 'A', (int) m, (int) n, reinterpret_cast<float*>(A.BufferPointer()), (int) lda, reinterpret_cast<float*>(SIGMA.BufferPointer()), reinterpret_cast<float*>(U.BufferPointer()), (int) ldu, reinterpret_cast<float*>(VT.BufferPointer()), (int) ldvt, &info);
#elif defined(USE_MKL)
        float wkopt;
        int lwork = -1;
        sgesvd("All", "All", &m, &n, reinterpret_cast<float*>(A.BufferPointer()), &lda, reinterpret_cast<float*>(SIGMA.BufferPointer()), reinterpret_cast<float*>(U.BufferPointer()), &ldu, reinterpret_cast<float*>(VT.BufferPointer()), &ldvt, &wkopt, &lwork, &info);
        lwork = (int) wkopt;
        W.Resize(lwork, 1);
        sgesvd("All", "All", &m, &n, reinterpret_cast<float*>(A.BufferPointer()), &lda, reinterpret_cast<float*>(SIGMA.BufferPointer()), reinterpret_cast<float*>(U.BufferPointer()), &ldu, reinterpret_cast<float*>(VT.BufferPointer()), &ldvt, reinterpret_cast<float*>(W.BufferPointer()), &lwork, &info);
#else
        std::vector<float> superb(std::max(std::min(m, n) - 1, 1));
        info = LAPACKE_sgesvd(BLAS_COLMAJOR 'A', 'A', (int) m, (int) n, reinterpret_cast<float*>(A.BufferPointer()), (int) lda, reinterpret_cast<float*>(SIGMA.BufferPointer()),
            reinterpret_cast<float*>(U.BufferPointer()), (int) ldu, reinterpret_cast<float*>(VT.BufferPointer()), (int) ldvt, &superb[0]);
#endif
    }
...
void CPUMatrix<ElemType>::ScaleAndAdd(ElemType alpha, const CPUMatrix<ElemType>& a, CPUMatrix<ElemType>& c)
{
    ...
    if (a.GetNumRows() != 1 && a.GetNumCols() != 1) // a is not a col or row vector
    {
        ...
        {
#ifdef USE_ACML
            daxpy(len, alpha, reinterpret_cast<double*>(a.BufferPointer()), incx, reinterpret_cast<double*>(c.BufferPointer()), incy);
#else
            cblas_daxpy(len, alpha, reinterpret_cast<double*>(a.BufferPointer()), incx, reinterpret_cast<double*>(c.BufferPointer()), incy);
#endif
        }
        else
        {
#pragma warning(suppress : 4244)
#ifdef USE_ACML
            saxpy(len, alpha, reinterpret_cast<float*>(a.BufferPointer()), incx, reinterpret_cast<float*>(c.BufferPointer()), incy);
#else
            cblas_saxpy(len, alpha, reinterpret_cast<float*>(a.BufferPointer()), incx, reinterpret_cast<float*>(c.BufferPointer()), incy);
#endif
        }
    }
...
#pragma omp parallel for
            ...
            {
#ifdef USE_ACML
                daxpy(m, alpha, reinterpret_cast<double*>(a.BufferPointer()), 1, reinterpret_cast<double*>(c.BufferPointer() + c.LocateColumn(j)), 1);
#else
                cblas_daxpy(m, alpha, reinterpret_cast<double*>(a.BufferPointer()), 1, reinterpret_cast<double*>(c.BufferPointer() + c.LocateColumn(j)), 1);
#endif
            }
        }
        else
...
#pragma warning(suppress : 4244)
#ifdef USE_ACML
                saxpy(m, alpha, reinterpret_cast<float*>(a.BufferPointer()), 1, reinterpret_cast<float*>(c.BufferPointer() + c.LocateColumn(j)), 1);
#else
                cblas_saxpy(m, alpha, reinterpret_cast<float*>(a.BufferPointer()), 1, reinterpret_cast<float*>(c.BufferPointer() + c.LocateColumn(j)), 1);
#endif
            }
        }
    }
...
#pragma omp parallel for
            ...
            {
#ifdef USE_ACML
                daxpy(n, alpha, reinterpret_cast<double*>(a.BufferPointer()), 1, reinterpret_cast<double*>(c.BufferPointer() + i), m);
#else
                cblas_daxpy(n, alpha, reinterpret_cast<double*>(a.BufferPointer()), 1, reinterpret_cast<double*>(c.BufferPointer() + i), m);
#endif
            }
        }
        else
...
#pragma warning(suppress : 4244)
#ifdef USE_ACML
                saxpy(n, alpha, reinterpret_cast<float*>(a.BufferPointer()), 1, reinterpret_cast<float*>(c.BufferPointer() + i), m);
#else
                cblas_saxpy(n, alpha, reinterpret_cast<float*>(a.BufferPointer()), 1, reinterpret_cast<float*>(c.BufferPointer() + i), m);
#endif
            }
        }
    }
...
#pragma omp parallel for
    ...
    for (long i = 0; i < (m & ~3); i += 4)
    {
        c.BufferPointer()[i] += alpha * (a.BufferPointer()[i] - b.BufferPointer()[i]);
        c.BufferPointer()[i + 1] += alpha * (a.BufferPointer()[i + 1] - b.BufferPointer()[i + 1]);
        c.BufferPointer()[i + 2] += alpha * (a.BufferPointer()[i + 2] - b.BufferPointer()[i + 2]);
        c.BufferPointer()[i + 3] += alpha * (a.BufferPointer()[i + 3] - b.BufferPointer()[i + 3]);
    }
    // handle remaining stuffs
    for (long i = m & ~3; i < m; i++)
    {
...
#pragma omp parallel for
    ...
    {
        c.BufferPointer()[i] = alpha * a.BufferPointer()[i];
        c.BufferPointer()[i + 1] = alpha * a.BufferPointer()[i + 1];
        c.BufferPointer()[i + 2] = alpha * a.BufferPointer()[i + 2];
        c.BufferPointer()[i + 3] = alpha * a.BufferPointer()[i + 3];
    }
    // handle remaining stuffs
...
void CPUMatrix<ElemType>::Scale(ElemType alpha, CPUMatrix<ElemType>& a)
{
    ...
    {
#ifdef USE_ACML
        dscal(len, alpha, reinterpret_cast<double*>(a.BufferPointer()), incx);
#else
        cblas_dscal(len, alpha, reinterpret_cast<double*>(a.BufferPointer()), incx);
#endif
    }
    else
    {
#pragma warning(suppress : 4244)
#ifdef USE_ACML
        sscal(len, alpha, reinterpret_cast<float*>(a.BufferPointer()), incx);
#else
        cblas_sscal(len, alpha, reinterpret_cast<float*>(a.BufferPointer()), incx);
#endif
    }
}
...
#pragma omp parallel for
            ...
            {
#ifdef USE_ACML
                c(0, j) = (ElemType) ddot(m, reinterpret_cast<double*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<double*>(b.BufferPointer() + b.LocateColumn(j)), 1);
#else
                c(0, j) = (ElemType) cblas_ddot(m, reinterpret_cast<double*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<double*>(b.BufferPointer() + b.LocateColumn(j)), 1);
#endif
            }
        }
        else
...
#pragma warning(suppress : 4244)
#ifdef USE_ACML
                c(0, j) = (ElemType) sdot(m, reinterpret_cast<float*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<float*>(b.BufferPointer() + b.LocateColumn(j)), 1);
#else
                c(0, j) = (ElemType) cblas_sdot(m, reinterpret_cast<float*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<float*>(b.BufferPointer() + b.LocateColumn(j)), 1);
#endif
            }
        }
    }
...
#pragma omp parallel for
            ...
            {
#ifdef USE_ACML
                c(i, 0) = ddot(n, reinterpret_cast<double*>(a.BufferPointer() + i), m, reinterpret_cast<double*>(b.BufferPointer() + i), m);
#else
                c(i, 0) = cblas_ddot(n, reinterpret_cast<double*>(a.BufferPointer() + i), m, reinterpret_cast<double*>(b.BufferPointer() + i), m);
#endif
            }
        }
        else
...
#pragma warning(suppress : 4244)
#ifdef USE_ACML
                c(i, 0) = sdot(n, reinterpret_cast<float*>(a.BufferPointer() + i), m, reinterpret_cast<float*>(b.BufferPointer() + i), m);
#else
                c(i, 0) = cblas_sdot(n, reinterpret_cast<float*>(a.BufferPointer() + i), m, reinterpret_cast<float*>(b.BufferPointer() + i), m);
#endif
            }
        }
    }
...
#else
        return (ElemType) cblas_ddot((int) a.GetNumElements(), reinterpret_cast<double*>(a.BufferPointer()), 1, reinterpret_cast<double*>(b.BufferPointer()), 1);
#endif
    }
    else
    {
...
#else
        return (ElemType) cblas_sdot((int) a.GetNumElements(), reinterpret_cast<float*>(a.BufferPointer()), 1, reinterpret_cast<float*>(b.BufferPointer()), 1);
#endif
    }
}
...
#pragma omp flush(bHas)
        if (!bHas)
        {
            ElemType cur = mat.BufferPointer()[j];
    ...
    else if (isColWise) // col-wise
    {
        ...
        if (sizeof(ElemType) == sizeof(double))
        {
            ...
            for (long j = 0; j < n; j++)
            {
                ...
                {
#ifdef USE_ACML
                    c(i, j) = (ElemType) ddot(m, reinterpret_cast<double*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<double*>(b.BufferPointer() + b.LocateColumn((j + shift + i - 1) % n)), 1);
#else
                    c(i, j) = (ElemType) cblas_ddot(m, reinterpret_cast<double*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<double*>(b.BufferPointer() + b.LocateColumn((j + shift + i - 1) % n)), 1);
#endif
                }
            }
        }
        else
        {
            ...
            for (long j = 0; j < n; j++)
            {
                ...
                {
#ifdef USE_ACML
                    c(i, j) = (ElemType) sdot(m, reinterpret_cast<float*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<float*>(b.BufferPointer() + b.LocateColumn((j + shift + i - 1) % n)), 1);
#else
                    c(i, j) = (ElemType) cblas_sdot(m, reinterpret_cast<float*>(a.BufferPointer() + a.LocateColumn(j)), 1, reinterpret_cast<float*>(b.BufferPointer() + b.LocateColumn((j + shift + i - 1) % n)), 1);
#endif
                }
            }
        }
...
#pragma omp parallel for
            ...
            {
#ifdef USE_ACML
                c(i, 0) = (ElemType) ddot(n, reinterpret_cast<double*>(a.BufferPointer() + i), m, reinterpret_cast<double*>(b.BufferPointer() + i), m);
#else
                c(i, 0) = (ElemType) cblas_ddot(n, reinterpret_cast<double*>(a.BufferPointer() + i), m, reinterpret_cast<double*>(b.BufferPointer() + i), m);
#endif
            }
        }
        else
...
ElemType CPUMatrix<ElemType>::LogAddSumOfElements() const
{
    ...
    for (int k = 0; k < GetNumElements(); k++)
        fAlpha = (ElemType) LogAddD(fAlpha, BufferPointer()[k]);
    return fAlpha;
}
...
#define CaseUnaryTensorOp(oper)                                                        \
    ...
    array<ElemType*, 2> pointers = {a.BufferPointer(), BufferPointer()};
    switch (op)
    {
...
#define CaseBinaryTensorOp(oper)                                                       \
    ...
    array<ElemType*, 3> pointers = {a.BufferPointer(), b.BufferPointer(), BufferPointer()};
    switch (op)
    {
...
#define CaseTernaryTensorOp(oper)                                                      \
    ...
    array<ElemType*, 4> pointers = {a.BufferPointer(), b.BufferPointer(), c.BufferPointer(), BufferPointer()};
    switch (op)
    {
